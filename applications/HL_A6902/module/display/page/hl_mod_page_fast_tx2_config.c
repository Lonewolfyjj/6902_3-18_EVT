/**
 * @file hl_mod_page_fast_tx2_config.c
...
{
    if (data_ptr->sys_status.tx2_record_state) {
        return HL_MOREONE_CHOOSE_LEFT;
    } else {
        return HL_MOREONE_CHOOSE_RIGHT;
    }
}
static hl_moreone_choose_t tx2_mute_switch(hl_display_screen_s* data_ptr)
{
    if (data_ptr->sys_status.tx2_mute_switch) {
        return HL_MOREONE_CHOOSE_LEFT;
    } else {
        return HL_MOREONE_CHOOSE_RIGHT;
    }
...
static hl_moreone_choose_t tx2_noise_switch(hl_display_screen_s* data_ptr)
{
    if (data_ptr->sys_status.tx2_noise) {
        return HL_MOREONE_CHOOSE_LEFT;
    } else {
        return HL_MOREONE_CHOOSE_RIGHT;
    }
...
static void hl_mod_page_setup(void)
{
    ...
    opt1_choose = tx2_record_switch(data_ptr);
    opt2_choose = tx2_mute_switch(data_ptr);
    opt3_choose = tx2_noise_switch(data_ptr);
