...
PAGE_DEC(PAGE_NOISE_REDUCTION_INTENSITY)
{
    ...
    result     = PageManager_PageRegister(PAGE_NOISE_REDUCTION_INTENSITY, hl_mod_page_setup, hl_mod_page_loop, hl_mod_page_exit,
                                      NULL);

    if (result == false) {
        LV_LOG_USER("PAGE_NOISE_REDUCTION_INTENSITY init fail\n");
    }
}
