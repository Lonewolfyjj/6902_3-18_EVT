...
static void bt1_cb(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_CLICKED) {        
        if(strcmp(ptr,lock)){
            ...
            lv_event_send(btn3,LV_EVENT_CLICKED,lock);
        }else{
...
static void bt3_cb(lv_event_t * e)
{
    char * ptr = (char *)lv_event_get_param(e);
    lv_event_code_t code = lv_event_get_code(e);
    ...
    if(code == LV_EVENT_CLICKED) {
        if(strcmp(ptr,lock)){
            LV_LOG_USER("btn3 Clicked\n");
            lv_obj_remove_style(btn3,&style_choose_main,LV_PART_MAIN);
            lv_obj_remove_style(btn_r,&style_choose_lr_main,LV_PART_MAIN);
            lv_obj_add_style(btn3,&style_choose_main,LV_PART_MAIN);
            lv_obj_add_style(btn_r,&style_choose_lr_main,LV_PART_MAIN);            
            lv_event_send(btn1,LV_EVENT_CLICKED,lock);
            lv_event_send(btn2,LV_EVENT_CLICKED,lock);
        }else{
            lv_obj_remove_style(btn3,&style_not_choose_main,LV_PART_MAIN);
            lv_obj_remove_style(btn_r,&style_not_choose_lr_main,LV_PART_MAIN);
            lv_obj_add_style(btn3,&style_not_choose_main,LV_PART_MAIN);
...
static void lv_page_4_init(void)
{
    ...
    btn3 = lv_btn_creat_fun(btn2,LV_ALIGN_OUT_RIGHT_MID,bt3_cb,0,0,80,54,0);
    btn_r = lv_btn_lr_creat_fun(btn3,LV_ALIGN_OUT_RIGHT_MID,-10,0,15,54,0);

    lab1 = lv_lab_creat_fun(btn1,btn1,LV_ALIGN_CENTER,0,0,"永不");
    lab2 = lv_lab_creat_fun(btn2,btn2,LV_ALIGN_CENTER,0,0,"15min");
    lab3 = lv_lab_creat_fun(btn3,btn3,LV_ALIGN_CENTER,0,0,"30min");
    lab4 = lv_lab_creat_fun(lv_scr_act(),btn2,LV_ALIGN_OUT_TOP_MID,0,-10,"自动关机");
...
lv_obj_t * hl_poweroff_obj_get(uint8_t num)
{
    return valid_obj[num];
}

//测试接口
void page_poweroff_test(void)
{
    // lv_style_page4_init();
    // lv_page_4_init();
}