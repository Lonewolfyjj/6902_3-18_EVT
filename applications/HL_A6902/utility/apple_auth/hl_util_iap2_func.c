    ...
    while (1) {
        switch (status) {
            case EM_HL_CHALLENGE_RESP_STM_WRITE_DATA:
                ...
                iap2->iap2_printf("\r\n%s:%d challenge_req_len = %d 111\r\n", __func__, __LINE__, challenge_req_len);
            ...
            case EM_HL_CHALLENGE_RESP_STM_READ_RESP_LEN:
                ...
                iap2->iap2_printf("\r\n%s:%d:%d:challenge_resp_len = %d\r\n", __func__, __LINE__, ret, challenge_resp_len);
...
int hl_iap2_identify_ack_challenge(st_iap2_protocol_p iap2)
{
    ...
    iap2->iap2_printf("[OK]Authentication Response 4444 [len = %d]\n", len);
...
int hl_iap2_identify_identification_info(st_iap2_protocol_p iap2)
{
    ...
    // packet param < 5 >
    param_len += hl_iap2_ctrl_add_param(addr+param_len, 5 + strlen(IAP2_FIRMWAREVERSION), 0x0004, IAP2_FIRMWAREVERSION);
    addr[param_len] = '\0';
    // addr += param_len;

    // packet param < 6 >
    param_len += hl_iap2_ctrl_add_param(addr+param_len, 5 + strlen(IAP2_HARDWAREVERSION), 0x0005, IAP2_HARDWAREVERSION);
    addr[param_len] = '\0';
    // addr += param_len;
    ...
    // packet param < 15 >
    param_len += hl_iap2_ctrl_add_param(addr+param_len, 5 + strlen(IAP2_UHOST_COMPONENT), 0x0001, IAP2_UHOST_COMPONENT);
    addr[param_len] = '\0';
    // addr += param_len;
