...
static hl_two_in_one_event_cb hl_two_in_one_func;
...
void hl_mod_two_in_one_ioctl(void * ctl_data)
{
    ...
    switch(ptr->two_in_one_choose){
        case HL_TWO_ONE_CHOOSE_LEFT:
            lv_event_send(btn_left,LV_EVENT_CLICKED,ulock);
        break;
        case HL_TWO_ONE_CHOOSE_RIGHT:
...
{
    hl_lvgl_two_in_one_init_t * ptr = (hl_lvgl_two_in_one_init_t *)init_data;
    hl_two_in_one_func = ptr->func_cb;
    if (!page_style_bit.page_two_in_one) {
        page_style_bit.page_two_in_one = 1;
        lv_style_page4_init();
    }
