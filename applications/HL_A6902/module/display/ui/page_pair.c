...
static void lv_page_4_init(void)
{
    ...
    lab1 = lv_lab_creat_fun(btn1,btn1,LV_ALIGN_CENTER,0,0,"取消");
    lab2 = lv_lab_creat_fun(btn2,btn2,LV_ALIGN_CENTER,0,0,"确定");
    lab3 = lv_lab_creat_fun(lv_scr_act(),lv_scr_act(),LV_ALIGN_TOP_MID,0,23,"是否进入配对");

    valid_obj[0] = btn1;
    valid_obj[1] = btn2;
}

lv_obj_t * hl_pair_obj_get(uint8_t num)
{
    return valid_obj[num];
}

//测试接口
void page_pair_test(void)
{
    // lv_style_page4_init();
    // lv_page_4_init();
}