...
static lv_obj_t * img21,*img22;
static lv_obj_t * lab1,*lab2,*lab3;



static void lv_img_hide_set(uint8_t chick_sta)
{    
    switch(chick_sta) {
        case CHICK_STA_LIFT: 
            lv_obj_add_flag(img12,LV_OBJ_FLAG_HIDDEN);
            lv_obj_clear_flag(img11,LV_OBJ_FLAG_HIDDEN);       

            lv_obj_add_flag(img21,LV_OBJ_FLAG_HIDDEN);
            lv_obj_clear_flag(img22,LV_OBJ_FLAG_HIDDEN);       
        break;
        case CHICK_STA_RIGHT:
            lv_obj_add_flag(img11,LV_OBJ_FLAG_HIDDEN);
            lv_obj_clear_flag(img12,LV_OBJ_FLAG_HIDDEN);       

            lv_obj_add_flag(img22,LV_OBJ_FLAG_HIDDEN);
...
static void lv_style_page4_init(void)
{
    ...
    lv_style_set_radius(&style_choose_main, 10);
...
static void lv_page_4_init(void)
{
    ...
    btn1 = lv_btn_creat_fun(bt1_cb,6,-6,138,82,0);
    btn2 = lv_btn_creat_fun(bt2_cb,150,-6,138,82,1);

    img11 = lv_img_creat_fun(btn1,&Other_high_true_black,10,0);
    img12 = lv_img_creat_fun(btn1,&Other_high_true_white,10,0);
    img21 = lv_img_creat_fun(btn2,&Other_sound_black,10,0);
    img22 = lv_img_creat_fun(btn2,&Other_sound_white,10,0);

    lab1 = lv_lab_creat_fun(btn1,img11,LV_ALIGN_OUT_RIGHT_MID,10,0,"高保真");
    lab2 = lv_lab_creat_fun(btn2,img21,LV_ALIGN_OUT_RIGHT_MID,10,0,"声音增强");
    lab3 = lv_lab_creat_fun(lv_scr_act(),lv_scr_act(),LV_ALIGN_TOP_MID,0,15,"音效模式");
...
lv_obj_t * hl_soundeffect_obj_get(uint8_t num)
{
    return valid_obj[num];
}

//测试接口
void page_soundeffect_test(void)
{
    // lv_style_page4_init();
    // lv_page_4_init();
}