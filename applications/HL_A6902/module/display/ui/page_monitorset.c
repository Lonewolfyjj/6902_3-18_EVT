...
static void lv_page_4_init(void)
{
    ...
    img11 = lv_img_creat_fun(btn1,&Other_monitor_uac_black,10,0);
    img12 = lv_img_creat_fun(btn1,&Other_monitor_uac_white,10,0);
    img21 = lv_img_creat_fun(btn2,&Other_monitor_tx_black,10,0);
    img22 = lv_img_creat_fun(btn2,&Other_monitor_tx_white,10,0);

    lab1 = lv_lab_creat_fun(btn1,img11,LV_ALIGN_OUT_RIGHT_MID,10,0,"UAC输入");
    lab2 = lv_lab_creat_fun(btn2,img21,LV_ALIGN_OUT_RIGHT_MID,10,0,"TX输入");
    lab3 = lv_lab_creat_fun(lv_scr_act(),lv_scr_act(),LV_ALIGN_TOP_MID,0,15,"监听设置");
...
lv_obj_t * hl_monitorset_obj_get(uint8_t num)
{
    return valid_obj[num];
}

//测试接口
void page_monitorset_test(void)
{
    // lv_style_page4_init();
    // lv_page_4_init();
}