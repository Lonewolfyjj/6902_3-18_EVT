                                           ...
                                           uint16_t timeout)
{
    ...
    msgs[1].addr  = 0x10;
    msgs[1].flags = RT_I2C_RD | RT_I2C_NO_READ_ACK | RT_I2C_IGNORE_NACK;
...
static int _hl_mod_apple_auth_iap2_cp_write(uint8_t reg_addr, uint8_t* write_data_addr, uint16_t write_data_len)
{
    ...
    msgs[1].buf   = write_data_addr;
    msgs[1].len   = write_data_len;

    int size = rt_i2c_transfer(apple_auth_chip_iic, msgs, 2);

    while (size != 2) {
        rt_kprintf("i2c write err!\n");
        size = rt_i2c_transfer(apple_auth_chip_iic, msgs, 2);
    }
