...
static void do_cap(void* arg)
{
    ...
    if (card_capture[0] == 0x00) {
        cap_config->card = rt_device_find("codecc");
    } else {
...
static void do_ply(void* arg)
{
    ...
    if (card_play[0] == 0x00) {
        ply_config->card = rt_device_find("codecp");
    } else {
...
#else
    cap_tid = rt_thread_create("do_start_audio", do_start_audio, RT_NULL, 2048, RT_THREAD_PRIORITY_MAX / 2, 1);
...
MSH_CMD_EXPORT(dsp_alango_loopback_enable_denoise, 1 : enable | 0 : disable);
