...
NAU88L25B_REG_T s_nau88l25b_param[] = {
    ...
    { 0x002B, 0x00D2 }, { 0x002C, 0x0080 }, { 0x002D, 0x0000 }, { 0x002F, 0x0000 }, { 0x0030, 0x0000 }, //0x00CF }, 新链路 
    ...
    { 0x0074, 0x0502 }, { 0x0076, 0x3140 }, { 0x0077, 0x0000 }, { 0x007F, 0x443F },  //0x433F }, 新链路 
...
static rt_err_t hl_nau88l25b_set_gain(struct audio_codec *codec, struct AUDIO_GAIN_CONFIG *dBConfig)
{
    ...
    rt_kprintf("nau88l25b_set_gain (%d)\n",  dBConfig->ch);
    /* Codec Driver Volume Control */
    if(dBConfig->ch == 1) {
...
static rt_err_t nau88l25b_reg_show(struct nau88l25b_priv* nau88l25b)
{
    ...
    rt_kprintf("NAU88L25B end reg show\r\n");
...
static rt_err_t nau88l25b_config(struct audio_codec* codec, eAUDIO_streamType stream, struct AUDIO_PARAMS* params)
{
    ...
    if (ret != RT_EOK)
        rt_kprintf("ERR: %s, something wrong: %d\n", __func__, ret);
#endif
    rt_kprintf("DBG: %s, nau88l25b config!\n", __func__);
...
int hl_nau88l25_test(int argc, char** argv)
{
    ...
    if (!strcmp("read", argv[1])) {
        ...
        if(nau88l25_param[0] == 0) {
            nau88l25b_set_gain_pga(nau88l25b_test, nau88l25_param[1], 0);
        } else {
            nau88l25b_set_gain_volume(nau88l25b_test, nau88l25_param[1], 0);
        }  
    } else if (!strcmp("show", argv[1])) {
        nau88l25b_reg_show(nau88l25b_test);
    } else {        
