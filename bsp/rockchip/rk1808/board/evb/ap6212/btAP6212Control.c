...
#ifdef RT_USING_BOARD_RK1808_EVB
#define BT_GPIO_PORT                (GPIO_BANK4)
#define BT_POWER_GPIO_PIN           GPIO_PIN_C7
#define BT_HOST_RX_PIN              GPIO_PIN_B4
#define BT_HOST_TX_PIN              GPIO_PIN_B5
#define BT_HOST_CTS_PIN             GPIO_PIN_B6
#define BT_HOST_RTS_PIN             GPIO_PIN_B7

#define GPIO_PORT_BASE              GPIO4
#define GPIO_BANK                   GPIO_BANK4
#define BT6212_UART_NAME       "/dev/uart4"
#endif
...
static void bt_hci_uart_hw_init(void)
{
    ...
    value = BUS_GRF->GPIO4B_IOMUX_H;
    rt_kprintf("uart hw init 0x%08X iomux = 0x%08X \n", BUS_GRF, value);

    /* config 32K */
    PMU_GRF->GPIO0C_IOMUX = 0xffff << 16 | (0x01 << 4);
    PMU_GRF->SOC_CON[0] = 0xffff << 16 | (0x01 << 0);
